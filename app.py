...
def show_venue(venue_id):
  ...
  if not venue:
    flash("Venue with ID "+ str(venue_id) +" was not found!", category='error')
...
def show_artist(artist_id):
  ...
  if not artist:
    flash("Artist with ID "+ str(artist_id) +" was not found!", category='error')
...
def edit_artist(artist_id):
  ...
  if not artist: 
    flash('An error occurred. Artist with ID ' + str(artist_id) + ' was not found!', category='error')
    return redirect(url_for('artists'))
...
def edit_artist_submission(artist_id):
  ...
  if not status:
    # on unsuccessful db insert, flash an error instead.
    flash('An error occurred. Artist ' + request.form['name'] + ' could not be edited.', category='error')
...
def edit_venue_submission(venue_id):
  ...
  if not status:
    # on unsuccessful db insert, flash an error instead.
    flash('An error occurred. Venue ' + request.form['name'] + ' could not be edited.', category='error')
...
def create_artist_submission():
  ...
  try:
    ...
    artist = Artist(name=name, city=city, state=state, phone=phone, image_link=image_link, facebook_link=facebook_link, genres=genres, website_link=website_link, seeking_venue=seeking_venue, seeking_description=seeking_description)
...
def create_show_submission():
  ...
  if not venue:
    flash('An error occurred. Show could not be listed. Venue ID '+str(venue_id)+' is invalid!')
    return redirect(url_for('index'))

  if not artist:
    flash('An error occurred. Show could not be listed. Artist ID '+str(artist_id)+' is invalid!')
    return redirect(url_for('index'))
